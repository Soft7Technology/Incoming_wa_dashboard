...
function runtime(mapping, owner = 'user') {
  ...
  vm.runInNewContext(js,{exports,require(id){assert.ok(dependencies[id], 'Unexpected runtime dependency: '+id); return dependencies[id];}});
...
test('runtime resolves active mapping and flow type without chat_bot reads', async () => {
  ...
  assert.equal(bot.id,'bot'); assert.equal(bot.user_id,'user'); assert.equal(bot.flow_type,'form');
});
